resource "aws_route_table" "OT367_public_rt" {
  vpc_id = aws_vpc.OT367_vpc.id

  tags = merge(var.common_tags, {
    Name = "OT-367-public-rt"
  })    
...
resource "aws_route_table_association" "OT367_public_rt_association" {
  subnet_id      = aws_subnet.OT367_public_subnet_1.id
  route_table_id = aws_route_table.OT367_public_rt.id
}
